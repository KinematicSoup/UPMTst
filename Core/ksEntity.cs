    ...
    public class ksEntity : ksBaseEntity
    {
        ...
        protected override void Initialize()
        {
            ...
            if (m_unityCharacterController != null)
            {
                ...
                if (colliderData != null && colliderData.Existance != ksExistanceModes.SERVER_ONLY)
                {
            ...
            if (rigidbody != null)
            {
                if (m_component.RigidBodyExistance == ksExistanceModes.SERVER_ONLY)
                {
                ...
                else
                {
                    ...
                    if (m_component.RigidBodyExistance == ksExistanceModes.CLIENT_AND_SERVER)
                    {
                        ...
                        foreach (ksIUnityCollider collider in m_component.GetColliders())
                        {
                            if (!collider.IsTrigger &&
                                (!collider.IsSimulationCollider || collider.Existance == ksExistanceModes.CLIENT_ONLY))
                            {
                    ...
                    if (rigidBodyType == ksRigidBodyModes.RIGID_BODY_2D)
                    {
                        RigidBody2D = new ksUnityRigidBody2DView(rigidbody, 
                            m_component.RigidBodyExistance == ksExistanceModes.CLIENT_AND_SERVER);
                    }
                    else
                    {
            ...
            foreach (ksColliderData colliderData in m_gameObject.GetComponents<ksColliderData>())
            {
                ...
                else if (colliderData.Existance == ksExistanceModes.SERVER_ONLY)
                {
        ...
        {
            return m_component.GetColliderData(collider, create);
        }
