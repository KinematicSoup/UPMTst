...
namespace KS.Reactor.Client.Unity
{
    ...
    public class ksInputManager : ksIInputManager
    {
        ...
        /// <summary>
        /// If false, all axes will be zero, all buttons will be up, and all values will be the default of their type.
        /// </summary>
        public bool Enabled
        {
        ...
        private class Value
        {
            ...
            /// <summary>
            /// Gets the value. Returns <see cref="DirectValue"/> if it is set and not undefined. Otherwise calls the
            /// <see cref="Getter"/> if it is set, or returns int 0.
            /// </summary>
            /// <returns>Value</returns>
            public ksMultiType GetValue()
            {
                if (DirectValue != null && DirectValue.Type != ksMultiType.Types.UNDEFINED)
                {
                    return DirectValue;
                }
        ...
        {
            Value value;
            if (m_valueMap.TryGetValue(valueId, out value))
            {
                ksMultiType val = value.GetValue();
                if (!m_enabled)
                {
                    return GetDefault(val.Type);
                }
                return val;
            }
            return 0;   
        ...
        /// <summary>Gets a <see cref="ksMultiType"/> with the default value of the given type.</summary>
        /// <param name="type">Type to get default value for.</param>
        /// <returns>Default value</returns>
        private ksMultiType GetDefault(ksMultiType.Types type)
        {
            switch (type)
            {
                case ksMultiType.Types.INT: return 0;
                case ksMultiType.Types.FLOAT: return 0f;
                case ksMultiType.Types.STRING: return (string)null;
                case ksMultiType.Types.BOOL: return false;
                case ksMultiType.Types.UINT: return 0u;
                case ksMultiType.Types.BYTE: return (byte)0;
                case ksMultiType.Types.LONG: return (long)0;
                case ksMultiType.Types.OBJECT: return new ksMultiType((ksIBufferable)null);
                default:
                {
                    ksMultiType value = new ksMultiType((byte[])null);
                    value.Type = type;
                    return value;
                }
            }
